...
FPR and FNR are considered source-specific assessment [@inconclusives2021]. Both of these are probabilities to make a false conclusion given  ground truth. These error rates are particular useful when we evaluate the process of of decision-making or an algorithm. FIR and FER are considered decision-specific assessment [@inconclusives2021]. These two probabilities assess the risk of a false claim given a decision. These probabilities are useful in real cases when a jury is asked to decide on a defendant's guilt after being presented with conclusions from forensic evidence. \yg{Theoretically, these error rates are calculated as conditional probablilities based on the beta mixture distributions set up in the previous section.}
...
\hh{XXX The next sentence needs to be integrated better into the writeup. You also need the continuous score.}
\yg{The calculation of the error rates is straightforward with the beta mixture distribution model.}


Error rates are also affected by the threshold used to make final decisions \hh{XXX thresholds only come into play when there is a latent continuous score in play. Bring that in, then talk about calculating error rates.}. In an extreme example, if we are very conservative in making identifications, we can set the threshold to be large enough, say CCF >= 0.99, then the FPR is 0. But at the same time, the FNR is 1. It's clear that there are trade-offs between the error rates. So, to make these error rates comparable and consistent, for the current paper, we refer the threshold as the value where the probability densities of KM and KNM distributions equal to each other. This is not a bad choice, since it has good statistical meaning that at this value, the likelihood ratio equals 1 and any deviation from 1 would give preference to a certain decision. 
...
For the following sections of the paper, we will base our analysis on a large reference database of four firings from each of 625 Beretta 92 F/FS collected by Srinivasan Rathinam from the Los Angeles Police Department. Subsequently, high-resolution 3d topographic scans of the land-engraved areas of these bullets were acquired at the Iowa State Roy J.\ Carver Microscopy facility. These scans form the basis of the investigation here and will be referred to as the **LAPD data**. These scans are publicly available through the NIST Ballistics Toolmark Research Database [@nist].
