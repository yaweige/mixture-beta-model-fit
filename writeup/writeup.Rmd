...
The error rates are also affected by the threshold selected to make decisions. For example, if we are very conservative in making identifications, we can set the threshold to be large, say CCF >= 0.9, then the FPR will be .... But at the same time, the FNR will be higher. There are trade-offs between the two error rates. So, to make these error rates comparable and consistent, for the current paper, we refer the threshold as the value where the probability densities of KM and KNM distributions equal with each other. For the two component beta mixture distributions we estimated in last section, this threshold is 0.529.

The four error rates are directly associated with the distributions we estimated as shown in the figures [figures illustrating this idea].
