...
Here, we focus on  four commonly used error rates as defined in \autoref{Error rate table}:
...
The probabilities of making the four decisions in \autoref{Error rate table} are then simply expressed as conditional probabilities:
\begin{eqnarray*}
...
\hh{The approach described below works in both of these scenarios. }
...
\hh{For the remainder of this paper we are going to assume that the comparison under investigation is whether a questioned bullet originated from a particular firearm. 
We will discuss the applicability of our method on situations without a suspected firearm at the end. XXX}



We distinguish between two scenarios depending on the reference used for comparisons:
\begin{enumerate}
\item case-specific: use scores from comparisons of test fires to estimate densities for the same source and the different source scores, and 
\item reference database: use scores with known provenance from a database of similar ammunition and firearms to estimate densities. 
\end{enumerate}
<!-- The second scenario is where our analysis of the theoretical distributions of the relevant population of similarity scores stand out.  -->

In either of the two scenarios, we derive scores by comparing test fires and the questioned bullet. 

\hh{XXX Let's sketch out this process separately for each scenario and describe the reference data/distribution in each case.}

The pipeline of the process is as follows: 

<!-- For a questioned bullet and a suspected firearm,  -->
...
<!-- \hh{XXX instead of SAM, use the 'in-phase' citation from NIST} -->
<!-- 4. Score-based LR are then calculated as ... -->

Then, for the first scenario, the KM and KNM distributions are estimated with the KM and KNM similarity scores among the test fired bullets respectively. While for the second scenario, the distributions are estimated with a database of the similarity scores from similar firearms and ammunition where all four bullets in each set are considered as test fired with a known source to fit the above pipeline.
