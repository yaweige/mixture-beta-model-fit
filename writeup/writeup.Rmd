...
\yg{The beta distributions are flexible in capturing unimodal asymmetric densities on the $[0,1]$ interval.}
...
<!-- we introduce a weighted sum of prior probabilities to combine several beta densities as one: -->
...
FPR and FNR are considered source-specific assessment [@inconclusives2021]. Both of these are probabilities to make a false conclusion given  ground truth. These error rates are particular useful when we evaluate the process of of decision-making or an algorithm. FIR and FER are considered decision-specific assessment [@inconclusives2021]. These two rates assess the risk of a false claim given a decision. They are particularly useful in situations, such as when a jury is asked to decide on a defendant's guilt after being presented with conclusions from forensic evidence. 

\yg{Theoretically, these error rates are calculated as conditional probabilities based on the beta mixture distributions set up in the previous section by introducing a threshold $c$ for the CCF values, that distinguishes between identifications ($CCF > c$) and eliminations ($CCF < c$). 
}
...
\hh{The four decisions in \autoref{Error rate table} are then simply calculated   as:
\begin{eqnarray*}
TP &=& P(CCF > c \mid \text{same source}) \\
FP &=& P(CCF > c \mid \text{different source}) \\
FN &=& P(CCF < c \mid \text{same source}) \\
TN &=& P(CCF < c \mid \text{different source}). \\
\end{eqnarray*}
}


Error rates are \hh{therefore} affected by the threshold $c$ used to make decisions. \hh{Error rates are also depending on each other, i.e. we can not optimize all at the same time, but we have to make trade-offs. } In an extreme example, we \hh{can set a} very conservative threshold of $CCF >= 0.99$ to make identifications. \hh{This yields a False positive rate (FPR) of zero.} But at the same time, the False negative rate (FNR) approaches 1. 
To make  error rates comparable and consistent we \hh{base the decision threshold here on the likelihood ratio and use} threshold $c_{X}$ where the probability densities of the KM and KNM distributions cross each other. At this crossing threshold $c_X$, the likelihood ratio equals 1. 
\hh{XXX do we have citations to the literature where this threshold is used?}
<!-- This is not a bad choice, since it has good statistical meaning that at this value, the likelihood ratio equals 1 and any deviation from 1 would give preference to a certain decision. -->

<!-- For the two component beta mixture distributions we estimated in last section, this threshold is `r round(cutoff1, digits = 2)`.  -->
...
Our analysis is based on a large reference database of four firings from each of 625 Beretta 92 F/FS collected by Srinivasan Rathinam from the Los Angeles Police Department. 
...
We distinguish between two scenarios \hh{depending on the reference used for comparisons}: \hh{XXX give these scenarios names}
\begin{enumerate}
\item \hh{case-specific:} use scores from comparisons of test fires to estimate densities for same source and different source, and 
\item \hh{reference database:} use scores with known provenance from a database of similar ammunition and firearms to estimate densities. 
\end{enumerate}
...
\hh{XXX move this paragraph up}
In either of the two scenarios, we derive scores by comparing test fires and the questioned bullet. The pipeline of the process is as follows: 
...
2. calculate reference distributions of KMs and KNMs by comparing all lands from pairs of test fires. In our example, six land-to-land comparisons correspond to known matches, the other 30 correspond to known non-matches for each pair of test fired bullets. 
\hh{Note, we can assume that all comparisons involving test fires are already included in the reference database. }
3. get comparison scores for the questioned bullet compared to each one of the test-fired bullets. We will designate the six scores identified by SAM as the best possible phase-match as 'candidate KM'. \hh{XXX we discard the rest - we could add a discussion item at the end on this. }\hh{XXX instead of SAM, use the 'in-phase' citation from NIST}
4. Score-based LR are then calculated as ...

\hh{XXX Notes: case-specific advantage: the only firarm involved is the suspected firearm. Distributions of scores should be close, as long as the ammunition matches the questioned bullet. (removing any  variability due to different firearms)
disadvantage: we need the firearm; there are not many scores that the densitiy is based -suffers both from over-fitting and possibly high variability (within firearm). Ideally we would like to get a lot of reference shots - unrealistic, time-consuming and cost-expensive.}
...
```{r one-two-component-betamix, out.width="70%", fig.align="center", fig.cap="Estimated beta mixture distributions for CCFs", fig.keep="hold", fig.pos="h"}
