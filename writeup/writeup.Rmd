...
Due to  limitations of sample sizes and study designs, sound distributional assumptions about the resulting similarity scores are not well established in the bullet comparison field for automated matching algorithms. Therefore, error rates are generally estimated empirically.
\hh{This estimation is often based on} only dozens of bullets from a selected few firearms. <!--originally used in case studies to estimate the current lab practice's error rates. -->
...
Any decision on the source of a comparison pair is then based on making a choice between those two  distributions. 
...
In this paper, we consider the cross-correlation function (CCF) between pairs of bullet signatures as an example for similarity scores. \hh{XXX this discussion was started above, let's move the next two sentences up.} The cross-correlation for bullet signatures has a practical range of 0-1. \yg{Although CCF values could theoretically be negative, that is almost never observed for LEA comparisons in practice (and in none of the more than 100k comparisons considered here).}

\hh{XXX move the next sentence up to 'signatures are extracted ...'}
The cross-correlation function (CCF) is calculated after extracting the signatures of bullet lands by maximizing the cross correlation of two sequence of signatures by horizontally adjusting the signatures. 
<!--\hh{XXX this sentence, too. }
 The larger CCF is, the stronger evidence is to support the same source assumption. HH: repeat from above-->
...
```{r example, out.width="70%", fig.align="center", fig.cap="Histograms of the KM and KNM of CCFs \\hh{XXX is there a reason why you don't use densities rather than the histograms?}", fig.keep="hold", fig.pos="h"}
myfit3n4_simulation %>%
...
comdata %>%
  ...
  geom_line() + 
#  scale_color_gradient2(low = "black", mid = "grey", high = "orange", midpoint = 11) + # HH: stay away from grey/orange unless you encode KM and KNM
  ylab("Density") + 
  xlab("Value") +
  guides(color = guide_none()) +
  facet_wrap(~as.numeric(id), labeller = "label_both")
