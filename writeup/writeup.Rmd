...
\begin{tabular}{ |p{1.8cm}||p{3.2cm} |p{3.1cm}| p{2.5cm}|}
 ...
 \hline
 Identification &  \bf{True Positives (TP)}: Correctly judged as matches & False Positive (FP): Incorrectly judged as matches& False Identification Rate $FIR = \frac{FP}{TP + FP}$ \\
 \hline
 Elimination & \bf{False Negative (FN)}: Incorrectly judged as non-matches& True Negative (TN): Correctly identified non-matches&False Exclusion Rate $FER = \frac{FN}{TN + FN}$ \\
 \hline
...
\begin{enumerate}
\item \hh{within-case:} use scores from comparisons of test fires to estimate densities for same source and different source, and 
...
1. given a questioned bullet and a suspected firearm, 
2. conduct test fires with the suspected firearm and collect three test fired bullets, 
3. for every pair of bullets (including the questioned), conduct all possible land comparisons and get 36 land comparison scores (6 land engraved areas on each bullet), 
4. among 36 comparisons within a pair of bullets,  select the six comparisons as KM comparisons (for comparisons among test fired bullets) or questioned land comparisons (for comparisons between the questioned bullet and any test fired bullet) by SAM [reference] due to the nature order of land engraved areas produced by a same barrels, \hh{XXX instead of SAM, use the 'in-phase' citation from NIST}
5.  KNM scores are only extracted from comparisons within the test fired bullets. 
