...
\yg{given a standard decision making procedure. In cases studied in the current paper, we make decisions by introducing a threshold $c$ for the CCF values, that distinguishes between identifications ($CCF > c$) and eliminations ($CCF < c$). }

<!-- by introducing a threshold $c$ for the CCF values, that distinguishes between identifications ($CCF > c$) and eliminations ($CCF < c$).  -->
...
\yg{The probabilities of making the four decisions in \autoref{Error rate table} are then simply expressed in conditional probability as:
\begin{eqnarray*}
...
\yg{The theoretical values of the four type of error rates are expressed in probability form as:
\begin{eqnarray*}
...
\yg{We estimate the reference distributions of KM and KNM based on the reference data. Denote the questioned comparison score as} $y$, the estimated KM and KNM distributions as $f(\cdot|KM)$ and $f(\cdot|KNM)$ respectively. The score-based likelihood ratios (SLR) are then calculated as:

$$SLR = \frac{f(y|KM)}{f(y|KNM)}$$
