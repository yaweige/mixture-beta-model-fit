...
\yg{XXX I think the transition here is a little abrupt, so I might add something like: In this paper, we explore those distributions by setting up models for the experimental data, and explore the estimation process of those models in real cases.}
<!-- \yg{By setting up distributional models, we could conduct estimations and control the error rates of the identification process.} -->
In this paper, we consider the cross-correlation function (CCF) between pairs of bullet signatures as an example for similarity scores. The cross-correlation for bullet signatures has a practical range of 0-1. \yg{Although CCF values could theoretically be negative, that is almost never observed for LEA comparisons in practice (and in none of the more than 100k comparisons considered here).}
...
The cross-correlation function (CCF) is calculated after extracting the signatures of bullet lands by maximizing the cross correlation of two sequence of signatures by horizontally adjusting the signatures [@vorburger2011]. CCF is the most prominent single feature in the automatic random forest algorithm [@Hare2016]. The larger CCF is, the stronger evidence is to support the same source assumption.
...
```{r example, out.width="70%", fig.align="center", fig.cap="Histograms of the KM and KNM of CCFs", fig.keep="hold", fig.pos="h"}
myfit3n4_simulation %>%
...
We can see from \autoref{fig:example} which is a typical \yg{pattern} we have for the CCFs of LEA comparisons, that the distributions of KM and KNM \yg{both have relatively high probabilities for values below and above 0.5. Therefore, there is not a clear separation of the histograms of the same-source and different-source comparisons of the LEA scans.}
<!-- \hh{XXX the overlap can be expressed in terms of a probability. By definition, the densities overlap. Only the empirical scores observed in practice might be separated.}.  -->
In the bullet LEA comparison problems, we usually have well separated bullet scores \yg{but not for the land scores according to} @Vanderplas2020. \yg{To capture the features of the distributions which are partly revealed in \autoref{fig:example}, we consider the beta distribution family to account for the unimodal pattern shown in the KNM distribution.}
...
```{r betamixFig, out.width="70%", fig.align="center", fig.cap="Two component beta-mixture distributions. From left to right, one component with (mu = 0.75, phi = 40) is held fixed, and the other component with (phi = 40) has mu changing from 0.25 to 0.75. The prior probablity p1 is fixed at 0.5. With the increase of the varying mu, the two modes gradualy merge to one", fig.pos="h"}
...
params <- data.frame(a = 10:30, b = 30:10)
comdata <- lapply(1:21, FUN = function(i) {
  output <- simdata(p1 = 0.5, a1 = params[i,1], b1 = params[i,2], a2 = 30, b2 = 10)
  output
}) %>% bind_rows(.id = "id") %>% filter((as.numeric(id) %% 2) == 1)

comdata %>%
  ggplot(aes(x = x, y = y, group = id, color = as.numeric(id))) + 
  geom_line() + 
  scale_color_gradient2(low = "black", mid = "grey", high = "orange", midpoint = 11) +
  ylab("Density") + 
  xlab("Value") +
